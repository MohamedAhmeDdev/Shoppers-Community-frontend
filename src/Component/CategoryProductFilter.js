...
function CategoryProductFilter({ filterProductList, error }) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filterProductList.length > 0 ? (
                        filterProductList.map((product, index) => {
                            const isAdded = wishListItems.find((item) => item.id === product.id);
                            return (
                                <div key={index} className="relative m-10 flex w-full max-w-xs flex-col overflow-hidden rounded-lg border shadow-md bg-white">
                                    ...
                                    <div className="mt-4 px-5 pb-5">
                                        ...
                                        {isAdded ? (
                                            ...
                                            <div onClick={() => removeFromWishList(product.id)} className="cursor-pointer w-full flex items-center justify-center rounded-md bg-red-500 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-red-700 focus:outline-none">
                                            ...
                                            <div class="flex justify-center items-center">
                                            <div onClick={() => addToWishList(product)} className="cursor-pointer w-full flex items-center justify-center rounded-md bg-blue-500 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-blue-700 focus:outline-none">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="h-6 w-6">
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
                                                </svg>
                                                {" "} Add to Wishlist
